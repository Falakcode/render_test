...
ECON_SCRAPE_INTERVAL = 3600  # Every hour

NEWS_SCRAPE_INTERVAL = 900   # Every 15 minutes
...
# ==================== FINANCIAL NEWS SCRAPER (WITH COST OPTIMIZATION) ====================
...
def optimize_articles_for_cost(articles: list) -> list:
    ...
    1. Removing similar/duplicate articles (70%+ similarity)

    2. Limiting articles per category

 

    Expected reduction: 50-70% fewer articles to classify
    ...
    # Step 1: Remove similar articles (70%+ similarity)
    ...
    for article in articles:
        ...
        for seen_title in seen_topics:
            ...
            if similarity >= 0.70:  # 70% similar = duplicate
    ...
    removed_similar = len(articles) - len(unique_articles)
    ...
    category_limits = {'crypto': 5, 'political': 3, 'central_bank': 3, 'market': 3}
...
async def financial_news_task():
    ...
    """Periodically scrape and classify financial news with cost optimization"""

    log.info("📰 Financial news scraper started (with cost optimization)")
    ...
    while not _stop.is_set():
        ...
        try:
            ...
            log.info("🔍 Starting news scan cycle")
            ...
            # COST OPTIMIZATION: Remove similar articles + apply category limits
            ...
            for article in filtered_articles:
                ...
                if not classification['is_important'] or classification['impact_level'] == 'low':
            ...
            log.info(f"😴 Waiting {NEWS_SCRAPE_INTERVAL // 60} minutes until next scan...")

            await asyncio.sleep(NEWS_SCRAPE_INTERVAL)
...
async def main():
    ...
    3. Financial news scraping (with cost optimization)
    ...
    log.info("   3️⃣ Financial news (15 RSS feeds + cost optimization)")
